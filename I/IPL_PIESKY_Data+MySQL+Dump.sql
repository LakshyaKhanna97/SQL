...
-- Q1
select bidder_id, 
		count(bid_status) as total_bids,
		sum(case when bid_status='Won' then 1 else 0 end)*100/
        sum(case when bid_status in ('Won','Bid','Lost')  then 1 else 0 end) as win_percentage
...
-- Q2
-- shows all the matches that were conducted and completed by the stadium 
...
b.city
 from ipl_match_schedule a
	join
 ipl_stadium b
 using(stadium_id) where a.status='Completed'
 group by b.stadium_name;

 -- shows all the matches that were conducted by the stadium irrespective of status
 select count(a.status),
 b.stadium_name,
 b.city
	from ipl_match_schedule a
...
sum(case when b.match_winner=b.toss_winner then 1 else 0 end)*100
/count(a.match_id) as percentage_win
	from ipl_match_schedule a
		join ipl_match b 
			using(match_id) 
		join ipl_stadium c 
...
group by a.stadium_id 
 order by c.stadium_name ;
 
 
 
 -- Q4
 -- total legit bids made by each team
 select a.bid_team ,
 b.Team_name,
 count(a.bidder_id) total_bids
	from ipl_bidding_details a 
 cross join 
 ipl_team b 
 where a.bid_team=b.team_id and 
 a.bid_status!='Cancelled'
 group by a.bid_team 
 order by a.bid_team;
 
 -- total bids made by team even if bids got cancelled
 select a.bid_team ,
 b.Team_name,
 count(a.bidder_id) total_bids
	from ipl_bidding_details a 
...
select a.match_id,
case when a.match_winner=1 then a.team_id1
	when a.match_winner=2 then a.team_id2 
		else a.MATCH_WINNER end as wining_team_id,
b.team_name winning_team,a.win_details
from ipl_match a 
cross join ipl_team b 
	where case  when a.match_winner=1 then a.team_id1=b.team_id
		when a.match_winner=2 then a.team_id2=b.team_id
			else a.MATCH_WINNER=b.team_id end;


 
-- Q6
select a.team_id,b.team_name,
sum(a.matches_played) Matches_played,
sum(a.matches_won) Matches_won,
sum(a.matches_lost) Matches_lost 
from
ipl_team_standings a 
join 
ipl_team b 
using(team_id) 
group by a.team_id;
...
select a.team_id,
b.team_name,
count(a.player_role) all_rounders
from ipl_team_players a 
join ipl_team b 
using(team_id)
